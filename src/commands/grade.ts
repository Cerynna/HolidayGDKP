...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (!result.ok && result.reason === 'no_grade' && result.character) {
      await interaction.editReply(
        `⚠️ Aucune perf exploitable pour **${result.character.name}**.\n${formatPerf(result.character, cfg)}`,
      );
      return;
    }
    ...
    if (result.character && result.resolved && result.applied) {
      ...
      await interaction.editReply(
        `${who}**${c.name}** : ${formatPerf(c, cfg)} ${change} (sur ${r.difficultyLabel})` +
          (elig ? `\n${elig}` : ''),
      );
    }
  } catch (err) {
    await interaction.editReply(`⚠️ Erreur : ${(err as Error).message}`);
  }
}
