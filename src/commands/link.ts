...
import type { WclMetric } from '../types.js';

export const data = new SlashCommandBuilder()
  .setName('link')
  .setDescription('Associe ton perso Warcraft Logs à ton compte Discord')
  .addStringOption((o) =>
    o.setName('perso').setDescription('Nom du personnage, ex: Thrall').setRequired(true),
  )
  .addStringOption((o) =>
    o.setName('royaume').setDescription('Royaume, ex: Hyjal').setRequired(true),
  )
  .addStringOption((o) =>
    o
      .setName('role')
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const wclMetric = (interaction.options.getString('role') ?? 'auto') as WclMetric;
