// Orchestration : à partir d'un membre lié, récupère la perf WCL (NM + HM),
// détermine le grade et applique le rôle.
...
import type { Config, CharacterPerformance, ResolvedGrade, Link, WclMetric } from './types.js';

export interface PerformanceResult {
  ok: boolean;
  reason?: 'not_linked' | 'no_grade';
  character?: CharacterPerformance;
  resolved?: ResolvedGrade | null;
  link?: Link;
  applied?: { added: boolean; roleName: string; changed: boolean };
  eligible?: boolean; // parse >= seuil d'accès au raid
}

/** Récupère les perfs (toutes difficultés) d'un membre lié, sans toucher aux rôles. */
export async function fetchMemberPerformance(member: GuildMember): Promise<PerformanceResult> {
...
export function formatPerf(
  ...
  for (const d of cfg.difficulties) {
    ...
    if (p) {
      const v = (p[metric as keyof typeof p] as number | undefined) ?? p.averageParse ?? p.bestOverall;
...
export function eligibilityLine(cfg: Config, eligible: boolean | undefined): string {
  if (!cfg.raidAccess) return '';
  return eligible
...
export async function processLink(
  ...
  try {
    ...
    if (result.ok && result.resolved && result.character) {
      ...
      return (
        `✅ Lié à **${c.name}-${server}** (${region.toUpperCase()}).\n` +
        `${formatPerf(c, cfg)}\n` +
    ...
    if (result.reason === 'no_grade' && result.character) {
      return (
        `✅ Lié à **${result.character.name}-${server}**, mais aucune perf exploitable pour attribuer un grade.\n` +
