...
client.once(Events.ClientReady, async (c) => {
  try {
    ...
    const wanted = new Map<string, Grade>();
    for (const g of allGrades(cfg.grades)) {
      if (!wanted.has(g.role.toLowerCase())) wanted.set(g.role.toLowerCase(), g);
    }
    ...
    if (cfg.raidAccess) {
      ...
      if (!exists) {
        ...
        if (!exists.hoist) await exists.edit({ hoist: true, reason: 'Accès raid GDKP' });
    ...
    if (ceiling <= ordered.length) {
      ...
      try {
        await guild.roles.setPositions(
          ordered.map((r, i) => ({ role: r.id, position: ceiling - 1 - i })),
        );
        console.log('📊 Rôles ordonnés (Valid en haut → Parfait → … → Commun).');
      } catch (err) {
        console.log('⚠️  Impossible de réordonner automatiquement :', (err as Error).message);
      }
    }

    console.log(`\nTerminé — ${created} créé(s), ${updated} mis à jour.`);
    console.log(
      '⚠️  Le rôle du bot doit rester AU-DESSUS de ces rôles (Paramètres serveur → Rôles).',
    );
  } catch (err) {
