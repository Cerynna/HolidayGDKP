...
export function allGradeRoleNames(cfg) {
    const tables = gradeTablesFor(cfg);
    const names = new Set();
    for (const table of Object.values(tables)) {
        for (const g of table)
            names.add(g.role.toLowerCase());
    }
    return names;
}
/**
 ...
 */
export function resolveGrade(cfg, character) {
    ...
    for (const key of priority) {
        ...
        if (p.bossesRanked < minBosses)
            continue;
        const score = p[metric] ?? p.averageParse ?? p.bestOverall;
 ...
 */
export async function applyGradeRole(guild, member, targetGrade) {
    const cfg = await getConfig();
    const gradeRoleNames = allGradeRoleNames(cfg);
    const targetName = targetGrade.role;
    const targetRole = guild.roles.cache.find((r) => r.name.toLowerCase() === targetName.toLowerCase());
    if (!targetRole) {
        throw new Error(`Le rôle "${targetName}" n'existe pas sur le serveur. Crée-le (nom exact) puis réessaie.`);
    }
    ...
    if (cfg.syncRoleColors && targetGrade.color) {
        ...
        if ((targetRole.hexColor || '').toUpperCase() !== wanted) {
            try {
                await targetRole.setColor(wanted, 'Couleur de parse WoW');
            }
            catch {
    ...
    const toRemove = member.roles.cache.filter((r) => gradeRoleNames.has(r.name.toLowerCase()) && r.id !== targetRole.id);
 ...
 */
export async function applyRaidAccessRole(guild, member, eligible, cfg) {
    if (!cfg.raidAccess)
        return { eligible, changed: false, missingRole: false };
    const role = guild.roles.cache.find((r) => r.name.toLowerCase() === cfg.raidAccess.role.toLowerCase());
    if (!role)
        return { eligible, changed: false, missingRole: true };
