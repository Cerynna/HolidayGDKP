...
export async function showRoleSelect(interaction) {
    const select = new StringSelectMenuBuilder()
        .setCustomId(ROLE_SELECT_ID)
        .setPlaceholder('Choisis ton rôle de jeu')
        .addOptions({ label: 'Auto (détection)', value: 'auto', emoji: '🔍', description: 'Le bot détecte DPS ou Heal' }, { label: 'DPS', value: 'dps', emoji: '⚔️' }, { label: 'Heal', value: 'hps', emoji: '💚' }, { label: 'Tank', value: 'tank', emoji: '🛡️' });
...
export async function showLinkModal(interaction) {
    ...
    const realm = new TextInputBuilder()
        ...
        .setStyle(TextInputStyle.Short)
        .setRequired(true);
    modal.addComponents(new ActionRowBuilder().addComponents(name), new ActionRowBuilder().addComponents(realm));
...
export async function handleLinkModal(interaction) {
    ...
    const wclMetric = (interaction.customId.split(':')[1] ?? 'auto');
    const name = interaction.fields.getTextInputValue(FIELD_NAME);
    const realm = interaction.fields.getTextInputValue(FIELD_REALM);
    const message = await processLink(interaction.guild, interaction.member, name, realm, null, // région : défaut de la config
    wclMetric);
